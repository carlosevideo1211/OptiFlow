...
interface Professional {
  ...
  tipo_pagamento?: string | null; valor_fixo?: number | null; percentual_comissao?: number | null;
}
...
function emptyProfForm() {
  return {
    ...
    tipo_pagamento:'', valor_fixo:null as number | null, percentual_comissao:null as number | null,
  };
}
...
export default function CadastrosPage() {
  ...
  const openEditProf = async (p: Professional) => {
    ...
    setProfForm({
      ...
      login_email:'', login_senha:'',
      tipo_pagamento:p.tipo_pagamento||'', valor_fixo:p.valor_fixo ?? null, percentual_comissao:p.percentual_comissao ?? null,
    });
  ...
  const handleSaveProf = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        tipo_pagamento: profForm.tipo_pagamento || null,
        valor_fixo: profForm.tipo_pagamento === 'fixo' ? profForm.valor_fixo : null,
        percentual_comissao: profForm.tipo_pagamento === 'comissao' ? profForm.percentual_comissao : null,
            ...
            <form onSubmit={handleSaveProf}>
              <div className="modal-body">
                ...
                <div style={{borderTop:'1px solid var(--border)',paddingTop:14,marginTop:14}}>
                  <div style={{fontSize:12,color:'var(--text-muted)',marginBottom:10}}>
                    Acerto financeiro por atendimento (usado no lançamento automático da Fase 6)
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                    <div>
                      <label className="form-label">Forma de pagamento</label>
                      <select className="form-input" value={profForm.tipo_pagamento} onChange={e=>setP('tipo_pagamento', e.target.value)}>
                        <option value="">Nenhum (não gera comissão)</option>
                        <option value="fixo">Valor fixo por atendimento</option>
                        <option value="comissao">Comissão (%)</option>
                      </select>
                    </div>
                    {profForm.tipo_pagamento === 'fixo' && (
                      <div>
                        <label className="form-label">Valor fixo (R$)</label>
                        <input className="form-input" type="number" step="0.01" value={profForm.valor_fixo ?? ''}
                          onChange={e=>setP('valor_fixo', e.target.value === '' ? null : parseFloat(e.target.value))} placeholder="0.00"/>
                      </div>
                    )}
                    {profForm.tipo_pagamento === 'comissao' && (
                      <div>
                        <label className="form-label">Percentual de comissão (%)</label>
                        <input className="form-input" type="number" step="0.01" value={profForm.percentual_comissao ?? ''}
                          onChange={e=>setP('percentual_comissao', e.target.value === '' ? null : parseFloat(e.target.value))} placeholder="0.00"/>
                      </div>
                    )}
                  </div>
                </div>
