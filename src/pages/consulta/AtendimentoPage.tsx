...
import {
  type Section, type Accordion,
  inp, num, fmtRx, validateRxField, validateRxRow,
  RxInput, Field, FInput, FTextarea, AccordionSection, OdOeGrid, ColHeader, RxTable,
} from './atendimentoUI';
...
export default function AtendimentoPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (data.valor_cobrado != null) {
        ...
        const { data: proc } = await supabase.from('procedures').select('default_price').eq('id', data.procedure_id).maybeSingle();
        if (proc?.default_price != null) setDocValorExame(String(Number(proc.default_price).toFixed(2)).replace('.', ','));
      }
      const { data: anexosData } = await supabase.from('customer_attachments')
        .select('id,file_name,file_path,created_at').eq('consultation_id', id).order('created_at', { ascending: false });
  ...
  const handleUploadAnexo = async (file: File) => {
    if (!tenantId || !consultation) return;
    setUploadingAnexo(true);
    try {
      const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
      const path = `receitas/${consultation.customer_id}/${Date.now()}_${safeName}`;
      const { error: upErr } = await supabase.storage.from('attachments').upload(path, file);
      if (upErr) throw upErr;
      const { error: insErr } = await supabase.from('customer_attachments').insert([{
        tenant_id: tenantId, customer_id: consultation.customer_id, consultation_id: id,
  ...
  const abrirAnexoAtendimento = async (a: any) => {
  ...
  // ── Fase 6: lançamentos financeiros automáticos ───────────────────────────────
  const syncLancamentosFinanceiros = async (consultationId: string, statusFinal: string) => {
    // Idempotente: sempre limpa os lançamentos antigos desta consulta antes de
    // recriar, assim salvar de novo (ex: corrigindo valor) não duplica nada.
    await supabase.from('clinic_financial_entries').delete().eq('consultation_id', consultationId);

    const valor = num(docValorExame);
    if (valor == null || valor <= 0 || statusFinal !== 'realizada') return;
    ...
    if (partnershipId) {
      const partner = partnerships.find(p => p.id === partnershipId);
    ...
    if (profId) {
      const { data: prof } = await supabase.from('professionals').select('tipo_pagamento,valor_fixo,percentual_comissao').eq('id', profId).maybeSingle();
      if (prof?.tipo_pagamento === 'fixo' && prof.valor_fixo) {
        entries.push({
          tenant_id: tenantId, type: 'despesa', category: 'comissao_profissional',
          description: 'Comissão do profissional (fixo)', amount: prof.valor_fixo,
          due_date: date || null, status: 'pendente',
          consultation_id: consultationId, professional_id: profId,
        });
      } else if (prof?.tipo_pagamento === 'comissao' && prof.percentual_comissao) {
        entries.push({
          tenant_id: tenantId, type: 'despesa', category: 'comissao_profissional',
          description: 'Comissão do profissional', amount: valor * prof.percentual_comissao / 100,
          due_date: date || null, status: 'pendente',
          consultation_id: consultationId, professional_id: profId,
        });
      }
    ...
    await supabase.from('clinic_financial_entries').insert(entries);
  };
  ...
  const handleSave = async (finalizando: boolean = false) => {
    ...
    try {
      ...
      if (finalizando) setStatus('realizada');
      toast.success(finalizando ? 'Atendimento finalizado!' : 'Salvo com sucesso!');
    } catch (err: any) {
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 120px)', overflowY: 'auto' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '0 0 40px' }}>
          ...
          {section === 'anexos' && (
            <div style={{ padding: '20px' }}>
              <h3 style={{ margin: '0 0 16px', fontSize: 15 }}>Anexos deste atendimento</h3>
              <div className="card" style={{ padding: 18 }}>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {anexos.length === 0 && (
                    <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Nenhum anexo ainda.</div>
                  )}
                  {anexos.map(a => (
                    <div key={a.id} onClick={() => abrirAnexoAtendimento(a)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,.04)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', fontSize: 12, cursor: 'pointer' }}>
                      <Paperclip size={13} /> {a.file_name}
                    </div>
                  ))}
                </div>
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'none', border: '1px dashed var(--border)', color: '#818cf8', borderRadius: 8, padding: '8px 12px', fontSize: 12, cursor: 'pointer', marginTop: 12 }}>
                  <Upload size={13} /> {uploadingAnexo ? 'Enviando...' : 'Anexar arquivo'}
                  <input type="file" accept="image/*,.pdf" style={{ display: 'none' }} disabled={uploadingAnexo}
                    onChange={e => { if (e.target.files?.[0]) handleUploadAnexo(e.target.files[0]); }} />
                </label>
              </div>
            </div>
          )}
