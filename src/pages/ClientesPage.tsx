...
<div style={{display:'flex',justifyContent:'space-between',fontWeight:700,fontSize:13,marginTop:4,borderTop:'1px solid var(--border)',paddingTop:4}}><span>Total:</span><span style={{color:'#22c55e'}}>{Number(o.total||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</span></div>
   ...
   const fg=(v:any,t:string='esf')=>{if(v===null||v===undefined||v==='')return'--';const n=parseFloat(v);if(isNaN(n))return'--';if(t==='eixo')return Math.round(n)+'\u00b0';if(t==='dnp')return n.toFixed(2).replace('.',',');if(t==='adicao')return n.toFixed(2).replace('.',',');const a=Math.abs(n).toFixed(2).replace('.',',');return(n>=0?'+':'-')+a;};
