...
export const imprimirCarne = async (v: Sale, storeSettings: StoreSettings | null) => {
  ...
  // Corrigido 06/09/2026 (Achado 9 da auditoria): antes, quando essa busca
  // voltava vazia (venda a vista/cartao sem crediario, falha de rede, RLS,
  // atraso de replicacao, ou um crediario que ainda nao teve suas parcelas
  // criadas), o codigo mais abaixo "inventava" datas de vencimento a partir
  // de hoje e imprimia um carne com datas sem nenhuma relacao com o
  // combinado com o cliente, sem avisar ninguem. Mesmo padrao ja corrigido
  // em crediarioDocumentos.ts (imprimirCarneCompleto): aborta e avisa o
  // operador em vez de imprimir dado fabricado.
  if (lista.length === 0) {
    if (v.payment_method !== 'crediario') {
      toast.error('Esta venda não possui parcelas de crediário — não há carnê para imprimir.', { duration: 6000 });
    } else {
      toast.error('Não foi possível carregar as parcelas deste carnê. Tente novamente em alguns instantes; se persistir, avise o suporte (venda #' + String(v.sale_number).padStart(4, '0') + ').', { duration: 8000 });
    }
    return;
  }
...
export const imprimirInstrumentoDivida = async (v: Sale, storeSettings: StoreSettings | null) => {
  ...
  const lista = (parc || []) as any[];
  // Corrigido 06/09/2026 (Achado 9 da auditoria): quando a venda tem mais de
  // uma parcela mas a busca das parcelas reais veio vazia (falha de rede,
  // RLS, atraso de replicacao, ou parcelas ainda nao criadas), o codigo mais
  // abaixo "inventava" datas de vencimento fake pro Demonstrativo de
  // Parcelamento deste documento juridico (confissao de divida) — ainda mais
  // grave aqui do que no carne, por ser um instrumento assinado pelo
  // cliente. Mesmo padrao ja corrigido em crediarioDocumentos.ts e em
  // imprimirCarne (acima): aborta e avisa o operador em vez de imprimir
  // dado fabricado.
  if (nP > 1 && lista.length === 0) {
    toast.error('Não foi possível carregar as parcelas desta venda para gerar o instrumento de dívida. Tente novamente em alguns instantes; se persistir, avise o suporte (venda #' + String(v.sale_number).padStart(4, '0') + ').', { duration: 8000 });
    return;
  }
  ...
  const parcelasDemo = lista.map((p: any, i: number) => '<div class="pd" style="'+(p.status==='pago'?'opacity:.55':'')+'"><div class="pl">Parcela '+(p.installment_number||i+1)+(p.status==='pago'?' — PAGA':'')+'</div><div class="pv">Venc: '+fmtD2(p.due_date)+'</div><div class="pa">'+fmtV(p.amount)+'</div></div>').join('');
