...
export default function AgendaPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('procedures').select('id,name,default_price').eq('tenant_id', tenantId).eq('active', true).order('name')
      .then(({ data }) => setProcedures(data || []));
    supabase.from('partnerships').select('id,name').eq('tenant_id', tenantId).eq('active', true).order('name')
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase.from('consultations').insert([{
        ...
        partnership_id: form.partnership_id || null,
            ...
            <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                {professionals.length > 0 ? (
                  <select className="form-input" value={form.professional_name} onChange={e => { const sel = professionals.find(pr => pr.name === e.target.value); set('professional_name', e.target.value); set('professional_id', sel?.id || ''); }}>
              ...
              <div>
                ...
                {procedures.length > 0 ? (
                  <select className="form-input" value={form.procedure_id} onChange={e => { const sel = procedures.find(pr => pr.id === e.target.value); set('procedure_id', e.target.value); set('procedure_type', sel?.name || ''); }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, alignItems: 'end' }}>
                <div>
                  ...
                  <select className="form-input" value={form.partnership_id} onChange={e => set('partnership_id', e.target.value)}>
                    <option value="">Particular</option>
                    {partnerships.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </div>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, paddingBottom: 8, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                  <input type="checkbox" checked={form.prioridade} onChange={e => set('prioridade', e.target.checked)} />
                  Prioridade
                </label>
              </div>
