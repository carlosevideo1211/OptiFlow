...
export default function VendasPage() {
  ...
  const finalizeSale = async () => {
    ...
    try {
      const { data: saleData, error: saleErr } = await supabase.from('sales').insert([{
        ...
        payment_method: payment, installments, subtotal, discount, total,
      ...
      if (payment === 'crediario' && selectedCustomer && installments > 0) {
        ...
        if (credData) {
          const parcelas = parcelasEdit.length === installments ? parcelasEdit : Array.from({ length: installments }, (_, i) => { const due = dueDate ? new Date(dueDate + 'T12:00:00') : new Date(); due.setMonth(due.getMonth() + i); return { amount: saldo/installments, due_date: due.toISOString().split('T')[0] }; });
      ...
      try { await supabase.from('financial_transactions').insert([{ tenant_id: tenantId, type: 'receita', description: 'Venda #' + saleData.sale_number + (customerName ? ' — ' + customerName : ''), category: 'Vendas', amount: total, due_date: new Date().toISOString().split('T')[0], paid_at: new Date().toISOString(), status: 'pago', payment_method: payment }]); } catch (_) {}
  ...
  const imprimirCarne = async (v: Sale) => {
    const { data: parc } = await supabase.from('crediario_parcelas').select('*').eq('sale_id', v.id).order('installment_number', { ascending: true });
